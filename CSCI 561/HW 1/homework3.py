...
def bfs(start, goal):
    ...
    while len(dq)!=0:
        ...
        for position in position_list:
            if position==goal: paths[ position ]  = pos; return paths
...
for goal in targets:
    path = algo_map[algo]((x,y), goal)
    if goal not in path: print('FAIL', file=fp)
    else: print(" ".join([ f"{c},{r}" for r,c in getPath(path, goal) ]), file=fp)
fp.close()