...
if case==1:
    ...
    rdd = sc.textFile(args[3].strip()) \
                    .filter(lambda x: x.split(',')[0] != 'user_id') \
                    .map(lambda x: tuple(x.split(','))) \
                    .map(lambda x: (x[1], frozenset({x[0]}))) \
                    .reduceByKey(lambda x, y : x|y ) \
                    .map(lambda x: x[1])
count = rdd.count()
...
def getOutputInFormat( itemset ):
    ...
    for length in range(1, mx+1):
        ...
        if length == 1: output.append(",".join( sorted([f"('{v[0]}')" for v in vals]) ) )
        else: output.append(",".join( sorted([f"{v}" for v in vals]) ) )
    return "\n\n".join(output)
